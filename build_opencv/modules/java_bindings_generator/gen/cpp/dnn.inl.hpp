...
#include "/Users/billalrahimi/Desktop/Robotics/C_Template/opencv/modules/dnn/misc/java/src/cpp/dnn_converters.hpp"
#include "/Users/billalrahimi/Desktop/Robotics/C_Template/opencv/modules/dnn/include/opencv2/dnn/dict.hpp"
#include "/Users/billalrahimi/Desktop/Robotics/C_Template/opencv/modules/dnn/include/opencv2/dnn/dnn.hpp"
#include "/Users/billalrahimi/Desktop/Robotics/C_Template/opencv/modules/dnn/include/opencv2/dnn/utils/inference_engine.hpp"
  ...
  (JNIEnv* env, jclass , jlong network_nativeObj)
{
    ...
    try {
        LOGD("%s", method_name);
        Ptr<cv::dnn::Model> _retval_ = makePtr<cv::dnn::Model>( (*(cv::dnn::Net*)network_nativeObj) );
        return (jlong)(new Ptr<cv::dnn::Model>(_retval_));
    } catch(const std::exception &e) {
        throwJavaException(env, &e, method_name);
    } catch (...) {
        throwJavaException(env, 0, method_name);
  ...
  (JNIEnv* env, jclass , jstring model, jstring config)
{
    ...
    try {
        ...
        const char* utf_model = env->GetStringUTFChars(model, 0); String n_model( utf_model ? utf_model : "" ); env->ReleaseStringUTFChars(model, utf_model);
        const char* utf_config = env->GetStringUTFChars(config, 0); String n_config( utf_config ? utf_config : "" ); env->ReleaseStringUTFChars(config, utf_config);
        Ptr<cv::dnn::Model> _retval_ = makePtr<cv::dnn::Model>( n_model, n_config );
        return (jlong)(new Ptr<cv::dnn::Model>(_retval_));
    } catch(const std::exception &e) {
        throwJavaException(env, &e, method_name);
    } catch (...) {
  ...
  (JNIEnv* env, jclass , jstring model)
{
    ...
    try {
        LOGD("%s", method_name);
        const char* utf_model = env->GetStringUTFChars(model, 0); String n_model( utf_model ? utf_model : "" ); env->ReleaseStringUTFChars(model, utf_model);
        Ptr<cv::dnn::Model> _retval_ = makePtr<cv::dnn::Model>( n_model );
        return (jlong)(new Ptr<cv::dnn::Model>(_retval_));
    } catch(const std::exception &e) {
        throwJavaException(env, &e, method_name);
    } catch (...) {
